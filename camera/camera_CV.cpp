...
Camera_Target_Alarm circle_found(){
	...
	for( size_t i = 0; i < circles.size(); i++ )
		...
		alarm_1.c_x = cvRound(circles[i][1]);
	}
}
...
World_frame coordinate_transformation( Camera_Target_Alarm alarm){
	...
	int C_calib_x = (535*alarm.c_x) + (0* alarm.c_y) + 311; 
	int C_calib_y = (0*alarm.c_x) + (528* alarm.c_y) + 231; 
	
	float X_world_calculation, Y_world_calculation; 
	
	X_world_calculation = 0.0032 + 0.6834 + exp((-1*(C_calib_x))/0.6796);
	...
	frame1.x_rob_frame = X_world_calculation;
	frame1.y_rob_frame = Y_world_calculation;
	return frame1; 
}

World_frame Find_Object_buff( void** buff, int width){
